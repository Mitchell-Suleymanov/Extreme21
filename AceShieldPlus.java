...
public class AceShieldPlus extends Ace {
	
	//Target will help in remove function. True = Player, False = Opponent
	boolean targetPlayer;

	public AceShieldPlus(){
		this.name = "Shield+";
		this.description = "Your bet is decreased by 2 while this Ace is in play.";
		this.isActive=true;
	}
	...
	public void use(Extreme21Game game){
		...
		if(game.getPlayerIsNext()){
			...
			game.getPlayer().setBet(bet-2); //Player used Shield+, so decrement Player's Bet by 2
			AceShieldPlus ace = new AceShieldPlus();
			game.getPlayer().getAcesInPlay().add(ace); //Add Shield+ to Player's list of acesInPlay
		}
		else{
			targetPlayer= false;			
			int bet = game.getOpponent().getBet();
			game.getOpponent().setBet(bet-2); //Opponent used Shield+, so decrement Opponent's Bet by 2 
			AceShieldPlus ace = new AceShieldPlus(); 
	...
	//Removal Effect: Increase your bet by 2
	@Override
